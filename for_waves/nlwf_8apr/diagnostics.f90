...
subroutine save_stats_chan(movie,final)
  ...
  do j = 1, Nyp
    do k = 0, Nzp - 1
      do i = 0, Nxm1
        s1(i, k, j) = u1(i, k, j) * th(i, k, j, 1)
        s2(i, k, j) = 0.5d0 * (u2(i, k, j + 1) + u2(i, k, j)) * th(i, k, j, 1)
      end do
    end do
  end do
  ...
  do j = 1, Nyp
    do k = 0, twoNkz
      do i = 0, Nxp - 1
        cs3(i, k, j) = (cnlwf_xz_dt(i, k, j) - cnlwf_xz_dtmem(i, k, j)) / dt
      end do
    end do
  end do
  ...
  do j = 1, Nyp
    do k = 0, twoNkz
      do i = 0, Nxp - 1
        cs4(i, k, j) = cs3(i, k, j) - cikx(i) * cikx(i) * cs1(i, k, j) &
                              - cikx(i) * (cs2(i, k, j + 1) - cs2(i, k, j - 1)) / (2.d0*gyf(j))
      end do
    end do
  end do
