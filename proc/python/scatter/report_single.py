...
from functions import get_metadata, read_params, get_grid, g2gf_1d, get_index, get_plotindex
...
im_scatter = ax[0,1].pcolormesh(sx, sy, scatter[-1], cmap='plasma')
...
sc_cb = plt.colorbar(im_scatter, cax=sc_cax, label=r"volume ($m^3$)")

flux_div = make_axes_locatable(ax[1,0])
flux_cax = flux_div.append_axes("right", size="5%", pad=0.05)
flux_cb = plt.colorbar(im_flux, cax=flux_cax, label=r"volume ($m^3$)")

corr_div = make_axes_locatable(ax[1,1])
corr_cax = corr_div.append_axes("right", size="5%", pad=0.05)
corr_cb = plt.colorbar(im_corrected, cax=corr_cax, label=r"volume ($m^3$)")

ax[0,0].set_title("(a) Plume cross-section")
ax[0,1].set_title("(b) $(b, \\phi)$ volume distribution $W$ in stratified layer")
ax[1,0].set_title("(c) $(b, \\phi)$ cumulative flux volume distribution $\\omega$ through $z=\
        \\frac{{9}}{{10}}H$")
ax[1,1].set_title("(d) Segregated volume distribution $\\Omega$ in stratified layer")

ax[0,1].set_xlabel(r"buoyancy ($m\,s^{-2}$)")
ax[0,1].set_ylabel("tracer conc.")
ax[0,1].set_xlim(bmin, bmax)
ax[0,1].set_ylim(tmin, tmax)
ax[1,0].set_xlabel(r"buoyancy ($m\,s^{-2}$)")
ax[1,0].set_ylabel("tracer conc.")
ax[1,0].set_xlim(bmin, bmax)
ax[1,0].set_ylim(tmin, tmax)
ax[1,1].set_xlabel(r"buoyancy ($m\,s^{-2}$)")
ax[1,1].set_ylabel("tracer conc.")
...
ax[1,1].axvline(b_zmax, color='k', linestyle='--', label="$N^2 (z_{{\\max}} - H)$")
...
ax[0,0].set_ylim(gz[idx_min], gz[idx_max-1])

plt.tight_layout()
plt.savefig('/home/cwp29/Documents/essay/figs/modifiedPDF.png',dpi=300)
plt.show()
