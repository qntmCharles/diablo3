...
fig, ax = plt.subplots(2, len(dirs), figsize=(6*len(dirs),8))
...
for d in range(len(dirs)):
    ...
    im_corrected = ax[1,d].pcolormesh(sx, sy, scatter_corrected[-1], cmap=s_cmap, norm=s_norm)

    test_array = np.zeros_like(t[-1])
    for i in range(int(md['Nb'])):
        for j in range(int(md['Nphi'])):
            test_array[np.logical_and(np.logical_and(b[-1] > bbin[i] - db/2,
                b[-1] <= bbin[i] + db/2),np.logical_and(t[-1] > tbin[j] - dt/2,
                t[-1] <= tbin[j] + dt/2))] = scatter_corrected[-1,j,i]

    trac_im = ax[0,d].pcolormesh(X, Y, test_array, cmap=s_cmap, norm=s_norm)
    b_cont = ax[0,d].contour(Xf, Yf, np.where(t[-1] <= 5e-4, b[-1], np.NaN),
                levels = contours_b, cmap='cool', alpha=0.8)
    b_cont_fill = ax[0,d].contourf(b_cont, levels = contours_b, cmap='cool', alpha=0.8, extend='min')
    t_cont = ax[0,d].contour(Xf, Yf, t[-1], levels = [5e-4], colors='green', alpha=0.8)
    ...
    ax[0,d].set_title("$N^2 = ${0:.2f}, t = {1:.2f} s".format(md['N2'], times[-1]))
