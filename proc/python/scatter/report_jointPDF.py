...
dirs = ["N2_1_nu_7e7", "N2_1_nu_7e7", "N2_1_nu_7e7"]
start_times = [5, 5, 5]
plot_times = [6, 10, 15]
...
for d in range(len(dirs)):
    ...
    bs.append(b)
    ts.append(t)
    scatters.append(scatter_corrected)
    scatter_ogs.append(scatter)
...
fig, ax = plt.subplots(2, len(dirs), figsize=(6*len(dirs),6))

for d in range(len(dirs)):
    scatter_corrected = scatters[d]
    scatter = scatter_ogs[d]
    b = bs[d]
    t = ts[d]
    md = mds[d]
    bbin = bbins[d]
    ...
    #########################################################
    # Create colour maps
    s_cvals = np.concatenate((np.array([-0.5*np.nanmax(scatter_corrected[plot_idx])]),
        np.linspace(0,np.nanmax(scatter_corrected[plot_idx]),31)[:]))
    ...
    im_corrected = ax[1,d].pcolormesh(sx, sy, scatter_corrected[plot_idx], cmap=s_cmap, norm=s_norm)

    test_array = np.zeros_like(t[plot_idx])
    for i in range(int(md['Nb'])):
        for j in range(int(md['Nphi'])):
            test_array[np.logical_and(np.logical_and(b[plot_idx] > bbin[i] - db/2,
                b[plot_idx] <= bbin[i] + db/2),np.logical_and(t[plot_idx] > tbin[j] - dt/2,
                t[plot_idx] <= tbin[j] + dt/2))] = scatter_corrected[plot_idx,j,i]

    trac_im = ax[0,d].pcolormesh(X, Y, test_array, cmap=s_cmap, norm=s_norm)
    ...
    t_cont = ax[0,d].contour(Xf, Yf, t[plot_idx], levels = [5e-4], colors='green', alpha=0.8)

    #########################################################

    sx_nan = sx[~np.isnan(scatter[plot_idx])].flatten()
    sy_nan = sy[~np.isnan(scatter[plot_idx])].flatten()
    ...
    bgrid, tgrid = np.meshgrid(bbin, tbin)

    b_com = np.nansum(bgrid * np.where(scatter_corrected[plot_idx] > 0, scatter_corrected[plot_idx], 0)) \
            / np.nansum(np.where(scatter_corrected[plot_idx]>0, scatter_corrected[plot_idx],0))
    t_com = np.nansum(tgrid * np.where(scatter_corrected[plot_idx] > 0, scatter_corrected[plot_idx], 0)) \
            / np.nansum(np.where(scatter_corrected[plot_idx]>0, scatter_corrected[plot_idx],0))

    ax[1,d].scatter(b_com, t_com, color='red', edgecolor='white', marker='^', s=200)
    ...
    ax[1,d].scatter(b_com, t_com, color='blue', edgecolor='white', marker='^', s=200)
    ...
    ax[0,d].set_title("$N^2 = ${0:.2f}, t = {1:.2f} s".format(md['N2'], times[plot_idx]))
