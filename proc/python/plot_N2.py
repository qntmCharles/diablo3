...
ims[0] = axs[0].imshow(th1_xz[0], cmap='bwr', interpolation='bicubic', animated=True,
        extent=[0,md['LX'],0,md['LZ']])
ims[1] = axs[1].imshow(w_xz_filtered[0], cmap='jet', interpolation='bicubic', animated=True,
        extent=[0,md['LY'],0,md['LZ']])
ims[2] = axs[2].imshow(N2t_xz_filtered[0], cmap='jet', interpolation='bicubic', animated=True,
        extent=[0,md['LY'],0,md['LZ']])
c_isopycnal = axs[0].contour(np.flip(th1_xz[0],axis=0), extent=[0,md['LY'],0,md['LZ']],
        levels=contour_isopycnals, colors='grey', alpha=0.5)
c_plume = axs[0].contour(np.flip(th1_xz[0],axis=0), extent=[0,md['LY'],0,md['LZ']], levels=contour_plume,
...
def animate(step):
    ...
    ims[0].set_data(th1_xz[step]-th1_xz[0]) #perturbation
    ims[1].set_data(w_xz_filtered[step])
    ims[2].set_data(N2t_xz_filtered[step])
    ...
    c_isopycnal = axs[0].contour(np.flip(th1_xz[step],axis=0), extent=[0,md['LY'],0,md['LZ']],
            levels=contour_isopycnals, colors='grey', alpha=0.5)
    c_plume = axs[0].contour(np.flip(th1_xz[step],axis=0), extent=[0,md['LY'],0,md['LZ']], levels=contour_plume,
            colors='grey',alpha=0.5)
