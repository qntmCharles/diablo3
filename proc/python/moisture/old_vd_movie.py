...
with h5py.File(join(save_dir,"movie.h5"), 'r') as f:
    ...
    scatter = np.array([np.array(f['b_phiv_W'][t]) for t in time_keys])
    scatter_flux = np.array([np.array(f['b_phiv_S'][t]) for t in time_keys])
...
print(t_orig.shape)
tracer_data_vert = t_orig[start_idx+1:, :, int(md['Nx']/2)]
tracer_thresh = 1e-5
...
while True:
    ...
    for z,c in zip(np.linspace(0.15, 0.3, Nz),cols):
        ...
        ax[0,1].plot(bs, md['q0'] * np.exp(md['alpha'] * (bs - md['beta'] * z)), color=c,
            label=r"$z={0:.2f} \, m$".format(z), alpha=0.5)
        ax[1,0].plot(bs, md['q0'] * np.exp(md['alpha'] * (bs - md['beta'] * z)), color=c,
            label=r"$z={0:.2f} \, m$".format(z), alpha=0.5)
        ax[1,1].plot(bs, md['q0'] * np.exp(md['alpha'] * (bs - md['beta'] * z)), color=c,
            label=r"$z={0:.2f} \, m$".format(z), alpha=0.5)

    ax[0,1].plot(bs, md['q0']*np.exp(md['alpha'] * (bs - md['beta']*heights[-1])), color='k', ls=':')
    ...
    def animate(step):
        ...
        ax[0,1].plot(bs, md['q0']*np.exp(md['alpha'] * (bs - md['beta']*heights[step])), color='k', ls=':')
        print(heights[step])
