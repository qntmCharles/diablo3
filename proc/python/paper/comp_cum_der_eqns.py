...
with h5py.File(join(save_dir, 'movie.h5'), 'r') as f:
    ...
    F_phi2_int = np.array([np.array(f['F_phimin_int'][t]) for t in time_keys])
    F_phi2_check = np.array([np.array(f['F_phimin'][t]) for t in time_keys])
...
for i in range(NSAMP):
    ...
    sum_vol.append(E[0])
    sum_vol2.append(np.sum(F_phi2_boundary[i]*db))
    ...
    #A_idx = np.argwhere(M[i, 0, :] > smooth_threshs[i])
    #if len(A_idx) == 0:
        #A_ent_vol.append(0)
    #else:
        #A_ent_vol.append(E[np.min(A_idx)] - T_ent_vol[-1])

    #U_ent_vol.append(E[0] - A_ent_vol[-1] - T_ent_vol[-1])
...
axs11[1].plot(times, sum_vol2, color='b', linestyle='--')
...
eps_im = axs12[0,0].pcolormesh(X, Y, eps[step], cmap='hot_r')
eps_contour_t = axs12[0,0].contour(Xf, Yf, th2_xz[step], levels=[tracer_thresh], colors='green', linestyles='--')
eps_cb = fig12.colorbar(eps_im, ax=axs12[0,0], label=r"$\varepsilon$")
eps_im.set_clim(0, 100)

chi_im = axs12[0,1].pcolormesh(X, Y, chi[step], cmap='hot_r')
#chi_contour_b = axs12[0,1].contour(Xf, Yf, b[step], levels=contour_lvls_b, colors='darkturquoise', alpha=0.7)
chi_contour_t = axs12[0,1].contour(Xf, Yf, th2_xz[step], levels=[tracer_thresh], colors='green', linestyles='--')

chi_cb = fig12.colorbar(chi_im, ax=axs12[0,1], label=r"$\chi$")
chi_im.set_clim(0, 0.1)

Reb_im = axs12[1,0].pcolormesh(X, Y, Re_b[step], cmap='hot_r')
#Reb_contour_b = axs12[1,0].contour(Xf, Yf, b[step], levels=contour_lvls_b, colors='darkturquoise', alpha=0.5)
Reb_contour_t = axs12[1,0].contour(Xf, Yf, th2_xz[step], levels=[tracer_thresh], colors='green', linestyles='--')

Reb_cb = fig12.colorbar(Reb_im, ax=axs12[1,0], label=r"$\mathrm{Re}_b$")
#Reb_im.set_clim(-1, 5)
Reb_im.set_clim(0, 20)

N2_im = axs12[1,1].pcolormesh(X, Y, N2[step], cmap='bwr')
#N2_contour_b = axs12[1,1].contour(Xf, Yf, b[step], levels=contour_lvls_b, colors='grey', alpha=0.5)
N2_contour_t = axs12[1,1].contour(Xf, Yf, th2_xz[step], levels=[tracer_thresh], colors='green', linestyles='--')
...
fig13, axs13 = plt.subplots(1, 2, figsize=(10, 3.5), constrained_layout=True)
...
axs14[1].set_xlabel(r"$\kappa_{\mathrm{eff}}$")
...
axs15[0].legend()
axs15[0].set_xlabel(r"Local buoyancy Reynolds number $\mathrm{Re}_b$")
