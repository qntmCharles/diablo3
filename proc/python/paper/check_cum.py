...
with h5py.File(join(save_dir, 'movie.h5'), 'r') as f:
    ...
    F_b_boundary = np.array([np.array(f['Ent_b_flux_int'][t]) for t in time_keys])
...
Wc = np.gradient(Wc, dphi, axis=1)
Wc = np.gradient(Wc, db, axis=2)
Wc *= db*dphi
Wc = np.where(Wc == 0, np.NaN, Wc)

Sc = np.gradient(Sc, dphi, axis=1)
Sc = np.gradient(Sc, db, axis=2)
Sc *= db*dphi
Sccum = np.copy(Sc)
for i in range(1, NSAMP):
    Sccum[i] += Sccum[i-1]
Sc = np.where(Sc == 0, np.NaN, Sc)
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# Figure 6.1: volume distribution Wc and source Sc evolution
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for d in range(len(steps)):
    ...
    im_W = axs[1,d].pcolormesh(sx, sy, Wc[steps[d]], cmap='plasma')
    axs[1,d].set_aspect(aspect)

    im_S = axs[2,d].pcolormesh(sx, sy, Sc[steps[d]], cmap='coolwarm', norm=S_norm)
    axs[2,d].set_aspect(aspect)
...
axs11[0].plot(times, np.nansum(Wc, axis=(1,2)), color='brown', ls=':')
axs11[0].plot(times, np.nansum(Sccum, axis=(1,2)), color='brown', ls=':')
