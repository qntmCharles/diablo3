...
for i in range(15, NSAMP):
    fig, ax = plt.subplots(1, 2)
    im = np.where(M[i] > 0, dWdt[i], np.nan)
    im = im[np.isfinite(im)]
    if len(im) == 0:
        print("fuck")
        continue
    w = filters.threshold_otsu(im) # w is a threshold for dw/dt

    hist, bins_center = exposure.histogram(im)
    ax[0].plot(bins_center, hist, lw=2)
    ax[0].axvline(w, color='k', ls='--')
    ax[0].set_title(times[i])

    nodes = [-np.nanmax(M[i]), 0, 0, m, m, np.nanmax(M[i])]
