...
with h5py.File(join(save_dir, 'movie.h5'), 'r') as f:
    ...
    F_b_int = np.array([np.array(f['Ent_b_flux_int'][t]) for t in time_keys])
    F_b_check = np.array([np.array(f['Ent_b_flux_rec'][t]) for t in time_keys])
...
for i in range(NSAMP):
    ...
    G = -np.gradient(F_b_boundary[i], db, axis=1)[0]

    sum_vol.append(E[0] + G[0])
...
axs11[1].plot(times, sum_vol, color='k', linestyle='--')
