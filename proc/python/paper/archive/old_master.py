...
fig5, ax5 = plt.subplots(1,2, figsize=(8, 3), constrained_layout=True)
...
ax5[0].set_xlim(3/T, times_trunc[-1]/T)
ax5[1].set_xlim(3/T, times_trunc[-1]/T)
...
for d in range(len(steps)):
    ...
    axs[1,d].axvline(md['N2']*heights[steps[d]]*L/B, linestyle=':', color='r')
...
axs8[0].pcolormesh(sx, sy, W[steps[-1]], cmap='plasma', alpha=0.5)

axs8[0].quiver(sxf[::2,::2], syf[::2, ::2], F_b[steps[-1], ::2, ::2],
        F_phi[steps[-1], ::2, ::2], angles='xy', units='xy', fc='k', ec='k', linewidth=0.1, scale=0.1)
...
cb_W = fig8.colorbar(im_W, ax = axs8[0], shrink=0.7)
cb_W.set_label("$W$", rotation=0, labelpad=10)
...
fig8, axs8 = plt.subplots(1,2, figsize=(8, 2.5), constrained_layout=True)
...
nodes = [-np.nanmax(M[steps[-1]]), 0, 0, Omega_thresh, Omega_thresh, np.nanmax(M[steps[-1]])]
...
im_b_edge = axs8[1].contour(Xf, Yf, plot_env[steps[-1]], levels = contours_b, cmap='cool', alpha=0.8)

im_M_bphi = axs8[0].pcolormesh(sx, sy, M[steps[-1]], cmap=custom_cmap, norm=norm)
im_M_xz = axs8[1].pcolormesh(X,Y, plot_array, cmap=custom_cmap, norm = norm)


axs8[0].set_title("(a) $(b,\phi)$-space")
axs8[0].set_ylabel(r"$\phi$", rotation=0, labelpad=10)
axs8[0].set_xlabel(r"$b$")
...
cbar_M = fig8.colorbar(im_M_bphi, ax=axs8[1], location="right", shrink=0.7)
cbar_M.set_label("$M$", rotation=0, labelpad=10)

cbar_env = fig8.colorbar(im_b_edge, ax=axs8[1], location='right', label=r"$b$", shrink=0.7)
cbar_env.set_label("$b$", rotation=0, labelpad=10)

axs8[1].set_title("(b) physical space")
axs8[1].set_ylabel("$z$", rotation=0, labelpad=10)
axs8[1].set_xlabel("$x$")

axs8[1].set_aspect(1)
axs8[1].set_xlim(-0.08/L, 0.08/L)
axs8[1].set_ylim(-0.6, 5.5)
