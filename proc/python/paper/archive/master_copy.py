...
boundary_width = phibins[1]-phibins[0]
F_phi_boundary = F_phi[:, 0, :]

boundary_width = 2*phibins[0]
bin_end = int(np.where(bbins == -1)[0][0])
bbins = bbins[1:bin_end]
bin_end = int(np.where(phibins == -1)[0][0])
phibins = phibins[1:bin_end]
...
E = total_vol - input_vol
print(np.min(np.argwhere(E > 0.5*input_vol)))
