...
with h5py.File(join(save_dir,out_file), 'r') as f:
    ...
    print("Loading field {0}".format("chi"))
    field = np.array(f['Timestep']["chi"][pvd_lim:-pvd_lim, idx_min:idx_max, pvd_lim:-pvd_lim])
    ...
    field -= np.log10(md['kappa'] + kappa_t) # remove kappa_eff factor

    kappa_t_zero = np.where(kappa_t == 0, kappa_t, np.nan)
    kappa_t_nonzero = np.where(kappa_t != 0, kappa_t, np.nan)

    field_lower_peak = field + np.log10(md['kappa'] + kappa_t_zero)
    field_upper_peak = field + np.log10(md['kappa'] + kappa_t_nonzero)

    kappa_eff = np.log10(md['kappa'] + kappa_t)
    h, bins = np.histogram(kappa_eff.flatten(), bins=256, range = (kappa_min, kappa_max), density=True)
    axs[1].semilogx(np.power(10,0.5*(bins[1:]+bins[:-1])), h, color='b', label=r"$\kappa + \kappa_T$")
    axs[1].axvline(md['kappa'], color='r', alpha=0.5, label=r"Prescribed $\kappa$")
    axs[1].legend()

    field += np.log10(md['kappa'] + kappa_t)
    ...
    axs[0].legend()
    axs[0].set_xlabel(r"Buoyancy variance dissipation rate $\chi$")
    axs[0].set_ylabel("PDF")
    axs[1].set_xlim(np.power(10, kappa_min), np.power(10, kappa_max))
    axs[1].set_ylim(0, 1)
    axs[1].set_xlabel(r"$\kappa_{\mathrm{eff}}$")
    axs[1].set_ylabel("PDF")

    #plt.savefig('/home/cwp29/Documents/papers/draft/figs/chi_pdf.png', dpi=300)
    #plt.savefig('/home/cwp29/Documents/papers/draft/figs/chi_pdf.pdf')
    plt.show()
