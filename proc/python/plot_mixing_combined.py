...
eps_im = ax[2,0].pcolormesh(X, Y, eps[-1], cmap='jet')
eps_contour_b = ax[2,0].contour(Xf, Yf, b[-1], levels=contour_lvls_b, colors='grey', alpha=0.5)
eps_contour_t = ax[2,0].contour(Xf, Yf, t[-1], levels=contour_lvls_t, colors='red', linestyles='--')
...
chi_contour_b = ax[0,1].contour(Xf, Yf, b[-1], levels=contour_lvls_b, colors='grey', alpha=0.5)
...
Ri_contour_b = ax[1,1].contour(Xf, Yf, b[-1], levels=contour_lvls_b, colors='grey', alpha=0.5)
...
def animate(step):
    ...
    global chi_contour_b, chi_contour_t, Ri_contour_b, Ri_contour_t, trac_contour_b, trac_contour_t
    ...
    trac_im.set_array(trac[step].ravel())
    ...
    for coll in Ri_contour_b.collections:
        coll.remove()
    for coll in trac_contour_t.collections:
        coll.remove()
    for coll in trac_contour_b.collections:
        coll.remove()
    ...
    return e_im, B_im, eps_im, chi_im, Ri_im, trac_im,
