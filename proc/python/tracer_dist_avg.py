...
ax2 = ax1.inset_axes([32, 1.3, 28, 1.2], transform=ax1.transData)
...
ax2.set_xlabel("time (non-dimensionalised)")
...
ax1.plot(source_dist, bbins_plot/b_nd, color='k', linestyle='--', label="pre-penetration")
...
for step,c in zip(t_inds, tcols):
    ...
    ax1.plot(np.mean(b_pdfs,axis=0), bbins_plot/b_nd, color=c, label = "t={0:.2f}".format(times[step]/t_nd))

ax1.set_ylabel("buoyancy (non-dimensionalised)")
ax1.set_xlabel("tracer (arbitrary units, normalised)")
ax1.set_ylim(0, 2.6)
ax1.set_xlim(0, 62)
#ax1.legend(loc='upper left', facecolor=(0.9,0.9,0.9), bbox_to_anchor=(0.1, 0.98))
ax1.legend(loc='upper left', bbox_to_anchor=(0.1, 0.98))
