...
fig_save_dir = '/home/cwp29/Documents/posters/dfd/figures'
...
S_bounds = np.linspace(-0.03,  0.03, 5)
...
for i in range(len(steps)):
    ...
    if i == len(steps)-1:
        ...
        cb_env.set_label("$b$", rotation=0, labelpad=10, fontsize=14)
        cb_plume = fig2.colorbar(im_plume, ax = axs2[i], location='right', shrink=0.7, extend='max')
        cb_plume.set_label("$\phi$", rotation = 0, labelpad=10, fontsize=14)
...
for d in range(len(steps)):
    ...
    if d == len(steps)-1:
        ...
        cb_W.set_label("$W$", rotation=0, labelpad=10, fontsize=14)
        cb_env = fig6.colorbar(im_b_edge, ax=axs[0,d], location='right', shrink=0.7)
        cb_env.set_label(r"$b$", rotation=0,  labelpad=10, fontsize=14)
        cb_plume = fig6.colorbar(im_phi, ax = axs[0,d], location='right', shrink=0.7, extend='max')
        cb_plume.set_label(r"$\phi$", rotation=0, labelpad=10, fontsize=14)
...
fig7, axs7 = plt.subplots(1,2, figsize=(8, 2.3), constrained_layout=True)
...
cbar_M.set_label("$M$", rotation=0, labelpad=10, fontsize=14)

cbar_env = fig7.colorbar(im_b_edge, ax=axs7[1], location='right', label=r"$b$", shrink=0.7)
cbar_env.set_label("$b$", rotation=0, labelpad=10, fontsize=14)
...
fig8, axs8 = plt.subplots(1,2, figsize=(8, 3.8), constrained_layout=True)

im_W = axs8[0].pcolormesh(sx, sy, W_thresh[steps[-1]], cmap='cool', alpha=0.7)
axs8[0].set_aspect(aspect)
im_W.set_clim(0, 0.25)

mask = ~np.isnan(W_thresh[steps[-1]])
...
if len(points) > 0:
    ...
    hull = spatial.ConvexHull(envelope)
    for simplex in hull.simplices:
        axs8[0].plot(envelope[simplex,0], envelope[simplex,1], 'r--', linewidth=0.6)

    axs8[0].plot([100, 100], [100, 100], 'r--', linewidth=0.6, label="Convex envelope")
    axs8[0].legend(loc="upper right")

cb_W = fig8.colorbar(im_W, ax = axs8[0], label=r"$W$", shrink=0.7, location='bottom')
cb_W.set_label("$W$", rotation=0, labelpad=7, fontsize=14)
...
axs8[0].set_ylabel(r"$\phi$", rotation=0, labelpad=7)
axs8[0].set_xlabel(r"$b$")

axs8[1].annotate("", xy=(0.2, 0.002), xytext=(3.8, 0.002),
        arrowprops=dict(arrowstyle="<->", connectionstyle="arc3", color="#0072CE"))
axs8[1].text(1.5, 0.005, r"Environmental fluid" "\n" r"($\phi = 0$)", fontsize=10, color="#0072CE")

axs8[1].annotate("", xy=(0.68, 0.01), xytext=(2.05, 0.04),
        arrowprops=dict(arrowstyle="<->", connectionstyle="arc3", color="#0072CE"))
axs8[1].text(1.7, 0.02, r"Undiluted plume" "\n" r"fluid ($\phi \propto b$)", fontsize=10, color="#0072CE")

axs8[1].text(0.1, 0.01, "Edge", fontsize=10, color="#0072CE", rotation=55)

axs8[1].text(1, 0.03, "Core", fontsize=10, color="#0072CE", rotation=55)

axs8[0].set_title(r"$W$ & $\mathbf{F}$ at $t=15$")
axs8[1].set_title(r"$S$ at $t=15$")
...
cb_S = fig6.colorbar(im_S, ax=axs8[1], label=r"$S$", shrink=0.7, location='bottom')
cb_S.set_label("$S$", rotation=0, labelpad=7, fontsize=14)
...
axs10[0].text(0.85, 0.03, r"$\mathcal{U}$", color="blue", fontsize=20)
axs10[0].text(2.6, 0.01, r"$\mathcal{T}$", color="green", fontsize=20)
axs10[0].text(1.8, 0.02, r"$\mathcal{A}$", color="red", fontsize=20)
axs10[0].annotate("", xy=(1.3, 0.007), xytext=(2, 0.018),
...
axs10[1].set_title("Physical space")
...
fig11, axs11 = plt.subplots(1, 3, figsize=(10, 2.5), constrained_layout=True)
...
axs11[0].plot(times[idx_QSS:], U_vol[idx_QSS:], color='b', label=r"$V(\mathcal{U})$")
axs11[0].plot(times[idx_QSS:], T_vol[idx_QSS:], color='g', label=r"$V(\mathcal{T})$")
axs11[0].plot(times[idx_QSS:], A_vol[idx_QSS:], color='r', label=r"$V(\mathcal{A})$")

axs11[0].set_xlabel(r"$t$")
axs11[0].set_title("Volume decomposition")
axs11[0].set_ylabel("Volume")
axs11[0].legend()

axs11[1].axvline(t_QSS, color='k', linestyle=':')
axs11[2].axvline(t_QSS, color='k', linestyle=':')
...
axs11[1].plot(times, U_ent_vol_early, color='b', alpha = 0.4)
axs11[1].plot(times, T_ent_vol_early, color='g', alpha = 0.4)
axs11[1].plot(times, A_ent_vol_early, color='r', alpha = 0.4)
...
#axs11[1].plot(times, sum_vol, color='k', label=r"$E$")
axs11[1].plot(times, total_vol - input_vol, label=r"$E$", color='k')

axs11[2].plot(times, U_ent_rate_early, color='b', alpha=0.4)
axs11[2].plot(times, T_ent_rate_early, color='g', alpha=0.4)
axs11[2].plot(times, A_ent_rate_early, color='r', alpha=0.4)

axs11[2].plot(times, U_ent_rate, color='b', label=r"$\dot{E}(\mathcal{U})/V(\mathcal{U})$")
axs11[2].plot(times, T_ent_rate, color='g', label=r"$\dot{E}(\mathcal{T})/V(\mathcal{T})$")
axs11[2].plot(times, A_ent_rate, color='r', label=r"$\dot{E}(\mathcal{A})/V(\mathcal{A})$")
#axs11[2].plot(times, sum_vol, color='r', linestyle='--')

axs11[2].set_title("Entrainment rate")
axs11[2].set_ylim(0, 1.5)
axs11[2].set_xlim(0, times[-1])
axs11[2].set_xlabel(r"$t$")
axs11[2].set_ylabel("Normalised volume flux")
axs11[2].legend()

axs11[1].set_title("Entrained volume")
axs11[1].set_xlim(0, times[-1])
axs11[1].set_ylim(0, 200)
...
axs12[0].set_title("Entrained volume flux")
...
axs12[1].set_title("Entrainment rate")
...
fig12, axs12 = plt.subplots(2, 2, figsize=(8, 3), constrained_layout=True)
...
axs12[1,0].set_xlabel(r"$x$")
axs12[1,1].set_xlabel(r"$x$")
axs12[0,0].set_ylabel(r"$z$")
axs12[1,0].set_ylabel(r"$z$")
...
eps_cb = fig12.colorbar(eps_im, ax=axs12[0,0], location='right', shrink=0.6)
eps_cb.set_label(r"$\varepsilon$", rotation=0, labelpad=10, fontsize=14)
eps_im.set_clim(0, 100)

chi_im = axs12[0,1].pcolormesh(X, Y, chi[steps[-1]], cmap='hot_r')
#chi_contour_b = axs12[0,1].contour(Xf, Yf, b[steps[-1]], levels=contour_lvls_b, colors='darkturquoise', alpha=0.4)
#chi_contour_t = axs12[0,1].contour(Xf, Yf, th2_xz[steps[-1]], levels=[tracer_thresh], colors='green', linestyles='--')

chi_cb = fig12.colorbar(chi_im, ax=axs12[0,1], location='right', shrink=0.6)
chi_cb.set_label(r"$\chi$", rotation=0, labelpad=10, fontsize=14)
...
Reb_cb = fig12.colorbar(Reb_im, ax=axs12[1,0], location='right', shrink=0.6)
Reb_cb.set_label(r"$I$", rotation=0, labelpad=10, fontsize=14)
#Reb_im.set_clim(-1, 5)
...
N2_cb = fig12.colorbar(N2_im, ax=axs12[1,1], location='right', shrink=0.6)
N2_cb.set_label(r"$\partial_z b$", rotation=0, labelpad=10, fontsize=14)
N2_im.set_clim(-2, 2)
...
print(avgs.T)
np.savetxt("/home/cwp29/Documents/posters/dfd/data.csv", avgs, delimiter=",", fmt="%2.2e")
