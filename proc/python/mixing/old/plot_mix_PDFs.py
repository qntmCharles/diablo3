...
ax = fig.subplot_mosaic("AABBCC;DEFGHI")

hist_bins = np.concatenate((np.linspace(np.nanmin(svd[step]), -5e-4, 10), np.array([0, 5e-4]),
        np.linspace(5e-4, np.nanmax(svd[step]), 10)[1:]))
N, bins, patches = ax["C"].hist(svd.flatten(), bins = hist_bins,
...
svd[step] = np.where(svd[step] > 5e-4, 1, svd[step])
svd[step] = np.where(svd[step] < -5e-4, -1, svd[step])
svd[step] = np.where(np.logical_and(svd[step] >= -5e-4, svd[step] <= 5e-4), 0, svd[step])
ax["B"].pcolormesh(sx, sy, svd[step], cmap=cmap)
ax["B"].set_aspect(bmax/phimax)

test_array = np.where(test_array > 5e-4, 1, test_array)
test_array = np.where(test_array < -5e-4, -1, test_array)
test_array = np.where(np.logical_and(test_array >= -5e-4, test_array <= 5e-4), 0, test_array)

#ax[1].pcolormesh(X, Y, test_array, cmap='coolwarm', norm=s_norm)
ax["A"].pcolormesh(X, Y, test_array, cmap=cmap)
phi_cont = ax["A"].contour(Xf, Yf, phi[step], levels = [5e-4], colors='k', alpha=0.8)
ax["A"].set_xlim(0.2, 0.4)
ax["A"].set_ylim(0.95*md['H'], 1.5*md['H'])
ax["A"].set_aspect(1)

fields = ["Ri", "chi", "Re_b", "tke", "e", "B"]
field_mins = [-1, 0, -4, 0, -2e-3, -3e-4]
field_maxs = [2, 2e-8, 15, 1e-7, 2e-3, 3e-4]
labels = [r"$\mathrm{Ri}$", r"$\chi$", r"$\log \mathrm{Re}_b$", r"$\log \varepsilon$", r"Diapycnal flux $e$",
        r"$B$"]

for i, key, l in zip(range(len(fields)), ["D", "E", "F", "G", "H", "I"], labels):
...
ax["A"].set_xlabel("x (m)")
ax["A"].set_ylabel("z (m)")
ax["A"].set_title(r"$\hat{\Omega}}$ coloured spatial field")

ax["B"].set_xlabel("buoyancy")
ax["B"].set_ylabel("tracer conc.")
ax["B"].set_title(r"Segregated volume distribution $\hat{\Omega}$")

mixing_line = mpatches.Patch(facecolor='g', edgecolor='g', label=r"$\left|\hat{\Omega}\right| < 0.0005$")
mixed_line = mpatches.Patch(facecolor='r', edgecolor='r', label=r"$\hat{\Omega} > 0.0005$")
plume_line = mpatches.Patch(facecolor='b', edgecolor='b', label=r"$\hat{\Omega} < -0.0005$")
ax["B"].legend(handles=[mixing_line, mixed_line, plume_line])


ax["C"].set_xlabel(r"$\hat{\Omega}$")
ax["C"].set_ylabel("Volume (normalised)")
ax["C"].set_title("Histogram of SVD values")
