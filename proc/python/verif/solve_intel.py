...
def F_stationary(t, y): return y[0]
F_stationary.terminal = True
F_stationary.direction = 1

u = [Qf(zstar), Mf(zstar)**2, Ff(zstar)]
t = zstar
tend = 0.3
...
while True:
    if flag:
        res = solve_ivp(rhs_m, (t, tend), u, events=[max_penetration_height, neutral_buoyancy_height,
            F_stationary])
    else:
        res = solve_ivp(rhs_p, (t, tend), u, events=[max_penetration_height, neutral_buoyancy_height,
            F_stationary])
    ts.append(res.t)
    ys.append(res.y)
    if res.status == 1:
...
for i in range(3):
    if i != 1:
        ...
        axs[i].plot(np.power(ys[i],1/2), ts)
