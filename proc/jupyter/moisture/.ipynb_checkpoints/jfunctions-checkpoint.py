...
def load_data(*vars, save_dir=None, verbose=True):
    if save_dir is None:
...
def load_mean_data(*vars, verbose=True):
    # Get directory locations from param file
    base_dir, run_dir, save_dir, version = read_params("./params.dat")
    
    with h5py.File(join(save_dir, 'mean.h5'), 'r') as f:
        if verbose: print("Mean keys: %s" %f.keys())
...
def load_az_data(*vars, save_dir=None, verbose=True):
    if save_dir is None:
        # Get directory locations from param file
        base_dir, run_dir, save_dir, version = read_params("./params.dat")

    with h5py.File(join(save_dir, 'az_stats.h5'), 'r') as f:
        if verbose: print("Az keys: %s" %f.keys())
...
def load_mean_bins(*vars, verbose=True):
    # Get directory locations from param file
    base_dir, run_dir, save_dir, version = read_params("./params.dat")
...
def load_bins(save_dir, *vars, verbose=True):
    with h5py.File(join(save_dir,"mean.h5"), 'r') as f:
        if verbose: print("Mean keys: %s" % f.keys())
        time_keys = list(f['tb_source'])
...
def load_bins_moist(save_dir, vapour, verbose=True):
    with h5py.File(join(save_dir,"mean.h5"), 'r') as f:
        if verbose: print("Mean keys: %s" % f.keys())
        time_keys = list(f['tb_source'])
...
def compute_F0(f, md, tstart_ind = 0, verbose=True, tracer=False, zbot=0.5, ztop=1.0, plot=False, moist=False, vapour=True):
    ...
    _, _, gzf, _ = get_grid(md, save_dir=f)
