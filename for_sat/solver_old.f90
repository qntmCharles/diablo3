    ...
    do n = 1, N_th
      ...
      if (n > 1) then
        do j = jstart_th(n), jend_th(n)
          do k = 0, Nzp - 1
            do i = 0, Nxm1
              if ((th(i,k,j,2) > q0 * exp( alpha_m * (th(i,k,j,1) - beta_m * gyf(j)))) .or. (th(i,k,j,3) > 0.d0)) then
                if (n == 2) then
                  s1(i,k,j) = s1(i,k,j) - ((q0 * exp(alpha_m * (th(i,k,j,1) - beta_m * gyf(j)))) - th(i,k,j,2))/tau_m
                else if (n == 3) then
                  s1(i,k,j) = s1(i,k,j) + ((q0 * exp(alpha_m * (th(i,k,j,1) - beta_m * gyf(j)))) - th(i,k,j,2))/ &
                          tau_m &
                          - w_s * (th(i,k,j+1,3)-th(i,k,j,3))/dyf(j+1)
                end if
               end if
            end do
          end do
...
subroutine rk_chan_1
  ...
  !switch off plume forcing at t=10s
  !if (time < 10.d0) then     
